...
import type { MatchupBuildResponse } from "./types.js";
...
export const parseMatchupResponse = (rawText: string): MatchupBuildResponse => {
	try {
		const jsonText = extractJsonText(rawText);
		return JSON.parse(jsonText) as MatchupBuildResponse;
	} catch (error) {
